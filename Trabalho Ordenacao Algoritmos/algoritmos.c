...
static void quickSortRecursivo(int vetor[], int inicio, int fim, Metricas* metricas, int tipoPivo) {
    if (inicio < fim) {
        // Estratégia de escolha de Pivô
        if (tipoPivo == 2) { // Mediana de 3
            int meio = inicio + (fim - inicio) / 2;
            metricas->comparacoes += 2; // Otimização: Apenas 2 comparações são necessárias para ordenar 3 elementos
            ...
            // Após as trocas, a mediana está no meio. Trocamos com o final para usar o mesmo 'particionar'.
            trocar(&vetor[meio], &vetor[fim], metricas);
        } else if (tipoPivo == 0) { // Pivô Central
            int meio = inicio + (fim - inicio) / 2;
            trocar(&vetor[meio], &vetor[fim], metricas);
        }
        // Para tipoPivo == 1 (Pivô Final), não fazemos nada, pois 'particionar' já usa o último.

        int pi = particionar(vetor, inicio, fim, metricas);

        // As chamadas recursivas agora passam o 'tipoPivo' adiante.
